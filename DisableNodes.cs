...
namespace DisableNodes
{
    class DisableNodes
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Usage: DisableNodes.exe <listfile>.txt");
                Environment.Exit(0);
            }

            string proddirector = Convert.ToString(000000000);
            ...
            {
                // Console.WriteLine("Disabling " + server);
                ...
                foreach (EngineDaemonInfo line in allinfo)
                {
                    foreach (Property a in line.Properties)
                    {
                        if (a.Name == "username")
                        {
                            if (a.Value == server)
                            {
                                Console.WriteLine("Disabling " + a.Value + ", Current: " + line.Enabled);
                                AdminManager.EngineDaemonAdmin.SetEnabled(line.EngineId, false);
